...
1. __Предобработка данных и EDA__
    ...
    значений и связей между таблицами. Оставили только треки, где были заполнены все четыре категории 
    жанр/артист/альбом/название трека. Из истории прослушиваний убрали строки, соответствующие удаленным трекам;
    - Для экономии ресурсов сократили датасет событий примерно в 3 раза с 5.4G до 1.5-2G, 
    ...
    - Также для экономии ресурсов оставили тип int32 у идентификаторов треков и пользователей в таблице interactions, соответствующую колонку в таблице tracks привели к этому же типу;
...
2. __Расчёт рекомендаций__
    ...
    - Рекомендации по умолчанию на основе топ-100 популярных треков по количеству прослушиваний:
        ...
        - Среднее покрытие "холодных" пользователей объектами: 1.87;
    - Персональные рекомендации на основе коллаборативного подхода и ALS 
    (сгенерировано по 50 рекомендаций для каждого пользователя):
        - coverage: 0.004,
        - novelty@5: 0.792,
        - precision: 0.0022,
        - recall: 0.0036;
    ...
    и жанровые оценки (сгенерировано по 50 рекомендаций для каждого пользователя):
        - coverage: 0.004,
        - novelty@5: 0.781,
        - precision: 0.0022,
        - recall: 0.0036.
