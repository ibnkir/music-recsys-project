...
сгенерированы рекомендации нескольких типов, получены метрики на тестовой выборке и разработан веб-сервис на базе FastAPI для получения смешанных персональных рекомендаций.
...
2. __Расчёт рекомендаций__
    ...
    Ниже перечислены реализованные подходы для генерации рекомендаций и их метрики на тестовой выборке:
    - Рекомендации по умолчанию на основе топ-100 популярных треков по количеству прослушиваний (baseline):
        - precision@5: 
        - recall@5: 
    ...
    и жанровые оценки (по 50 рекомендаций на каждого пользователя).Метрики на тестовой выборке:
        - precision@5: 0.0022
        - recall@5: 0.0036
        - coverage: 0.004
        - novelty@5: 0.781
        
    Также были рассчитаны контентные рекомендации с помощью ALS на основе жанровой близости треков для генерации онлайн-рекомендаций (по 10 на каждый трек).
