"""FastAPI-приложение для сохранения и получения последних событий, нужных для генерации онлайн-рекомендаций.

Метод /put сохраняет пару значений user_id и item_id как событие, а метод /get возвращает события (первыми — самые последние).
